...
- **Save any word yourself** — long-press (or right-click) any word in the reader to save it, even one the LLM left alone: “Save phrase…” then picks a multi-word term by tapping its other end. The translation is fetched on demand, in the sense the word carries in that sentence, and the word joins the same vocabulary as a tapped one. Saving works offline — the entry appears at once and its translation catches up on the next sync.
- **Comment threads, not just articles** — paste a Hacker News link (`news.ycombinator.com/item?id=…`) and the whole discussion is ingested as one text: every comment at any depth, nested under its author and translated like anything else. Fold a branch you have read (or one you would rather skip) and the fold is remembered — offline too, and on your other devices after the next sync. The library marks such a card with a discussion icon and can filter down to articles or discussions.
